...
class IPrefetchIO(implicit p: Parameters) extends IPrefetchBundle {
  ...
  val wayLookupFlushFromBpuS3 = Output(Bool())
}
...
class IPrefetchPipe(implicit p: Parameters) extends IPrefetchModule {
  ...
  val toWayLookupFtqIdx = RegEnable(s1_req_ftqIdx, toWayLookup.fire)
  io.wayLookupFlushFromBpuS3 := io.flushFromBpu.shouldFlushByStage3(toWayLookupFtqIdx)
