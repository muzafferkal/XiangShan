...
//    out.isFirstIssue  := src.isFirstIssue
  ...
  val s3_safe_writeback = RegEnable(s2_safe_writeback, s2_fire) || s3_hw_error
  ...
  io.fast_rep_out.bits.delayedLoadError := s3_hw_error
  ...
  when (s3_exception || s3_hw_error || s3_rep_frm_fetch) {
  ...
  s3_out.bits.uop.exceptionVec(hardwareError) := s3_hw_error && s3_vecActive
  ...
  if (EnableLoadToLoadForward) {
    ...
    io.l2l_fwd_out.dly_ld_err := s3_hw_error || // ecc delayed error
                                 s3_ldld_rep_inst ||
                                 s3_rep_frm_fetch
  } else {
